...
interface TopicAnalysisProps {
  topics?: Topic;  // Make it optional to avoid errors
...
const TopicAnalysis: React.FC<TopicAnalysisProps> = ({ topics, commonWords }) => {
  ...
  useEffect(() => {
    if (!topics?.distribution || !topicsChartRef.current) return;
              ...
              {Object.entries(topics?.distribution ?? {}).map(([topic, percentage], index) => (
                <div key={index} className="flex items-center">
                  <div
                    ...
                    style={{
                      backgroundColor: `rgba(${index * 50}, ${100 + index * 20}, ${200 - index * 20}, 0.8)`,
                    }}
                  ></div>
